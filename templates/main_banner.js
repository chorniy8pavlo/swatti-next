import classes from '../styles/main_banner.module.css'
import Link from 'next/link'

export function MainBanner(){
	return(
	<section className={classes.main_banner} >
			<Link href="/catalog"><div className={classes.banner_block} style={{backgroundImage:"url('http://localhost:3000/banner_left.jpg')"}}>
			
				<h2>ЖІНОЧI ПІЖАМИ</h2>
				<a><svg width="46" height="8" viewBox="0 0 46 8" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M45.3536 4.35355C45.5488 4.15829 45.5488 3.84171 45.3536 3.64645L42.1716 0.464466C41.9763 0.269204 41.6597 0.269204 41.4645 0.464466C41.2692 0.659728 41.2692 0.976311 41.4645 1.17157L44.2929 4L41.4645 6.82843C41.2692 7.02369 41.2692 7.34027 41.4645 7.53553C41.6597 7.7308 41.9763 7.7308 42.1716 7.53553L45.3536 4.35355ZM0 4.5H45V3.5H0V4.5Z" fill="#1E1D1D" />
					</svg></a>
			</div></Link> 
			<Link href="/catalog"><div className={classes.banner_block} style={{backgroundImage:"url('http://localhost:3000/banner_right.jpg')"}}>
				<h2>ЧОЛОВІЧІ ПІЖАМИ</h2>
				<a><svg width="46" height="8" viewBox="0 0 46 8" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M45.3536 4.35355C45.5488 4.15829 45.5488 3.84171 45.3536 3.64645L42.1716 0.464466C41.9763 0.269204 41.6597 0.269204 41.4645 0.464466C41.2692 0.659728 41.2692 0.976311 41.4645 1.17157L44.2929 4L41.4645 6.82843C41.2692 7.02369 41.2692 7.34027 41.4645 7.53553C41.6597 7.7308 41.9763 7.7308 42.1716 7.53553L45.3536 4.35355ZM0 4.5H45V3.5H0V4.5Z" fill="#1E1D1D" />
					</svg></a>
			</div>
		</Link> 
		</section>
		)
}